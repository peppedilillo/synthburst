...
def estimate_burst_counts(burst_pdf, factor):
    ...
    num = sum((model_pdf_counts / max(model_pdf_counts))
              * (model_pdf_bins[1:] - model_pdf_bins[:-1])
              * (_background_X.rate + _background_S.rate)) / factor
    return round(num)
...
def energies(l):
    return [(_instrument_X.en_bins[c], _instrument_X.en_bins[c + 1] ) if f == 'X' else
            (_instrument_S.en_bins[c], _instrument_S.en_bins[c + 1] ) for t,c,f in l]
...
def _make_events_list(time_tags, energies):
    ...
    """
    tupled_en_list = [(value, key) for key in energies.keys() for value in energies[key]]
...
class _LightCurveSim(_LightCurve):
    ...
    def add_bkg(self, counts='hermes_default',
                      is_flux=False,
                      bkg_model=None):
        """
    ...
    def add_burst(self, counts):
        ...
        :return:
        """
        time_tags = [time_tags + self.burst_start_time for time_tags in self._simulate_burst_tte(counts)]
    ...
    def _simulate_bkg_tte(self, counts, bkg_model = None):
        if bkg_model:
            ...
            time_tags, _ = generate_by_invsampl(_pdf(values / sum(values), bins), counts)
        else:
    ...
    def _sim_bkg_energies(N):
        ...
        def compute_photons_num_per_instrument():
            N_X = ceil(N * _background_X.rate / (_background_X.rate + _background_S.rate))
            N_S = floor(N * _background_S.rate / (_background_X.rate + _background_S.rate))
            return N_X, N_S

        out_dic = {}
        N_X, N_S = compute_photons_num_per_instrument()
        for instrument, background, num, key in list(zip(*[[_instrument_X, _instrument_S],
                                                           [_background_X, _background_S],
                                                           [N_X, N_S],
                                                           ['X', 'S']])):
            _, channel_nums = generate_by_invsampl(_pdf(background.counts / sum(background.counts),
                                                        instrument.en_bins),
                                                    num)
    ...
    def _sim_burst_energies(N, band):
        ...
        def convolvify(en_bins, arf, rmf_mat):
            ...
            mid_points = [(en_bins[i] + en_bins[i - 1]) / 2 for i in range(1, len(en_bins))]
        ...
        def compute_photons_num_per_instrument():
            ...
            for i, instrument in enumerate([_instrument_X, _instrument_S]):
                mid_points = [(instrument.en_bins[i] + instrument.en_bins[i - 1]) / 2 for i in
                              range(1, len(instrument.en_bins))]
                binwidths = [(instrument.en_bins[i] - instrument.en_bins[i - 1]) for i in
                              range(1, len(instrument.en_bins))]
                arfdotband = [instrument.arf(p) * band(p) * bw for p, bw in list(zip(mid_points, binwidths))]
                C[i] = sum(arfdotband)
            return ceil(N * C[0] / (C[0] + C[1])), floor(N * C[1] / (C[0] + C[1]))

        out_dic = {}
        N_X, N_S = compute_photons_num_per_instrument()
        for instrument, num, key in list(zip(*[[_instrument_X, _instrument_S],
                                               [N_X, N_S],
                                               ['X', 'S']])):
            counts_arr, mid_points = convolvify(instrument.en_bins, instrument.arf, instrument.rmf_mat)
            _, channel_nums = generate_by_invsampl(_pdf(counts_arr, instrument.en_bins), num)
            out_dic.setdefault(key, channel_nums)
        return out_dic
    ...
    def get_data(self, energy=None, time=None, flag=None):
        ...
        if energy:
            low, hi = energy
            condition = (lambda d:   low <= _instrument_S.en_bins[d[1]] < hi if d[2] == 'S'
    ...
    def get_burst_times(self):
        try:
            start = self.burst_start_time + floor(self.burst_pdf.bins[0]/self.binning)*self.binning
            end = self.burst_start_time + ceil(self.burst_pdf.bins[-1]/self.binning)*self.binning
            return start, end
    ...
    def plot(self, figsize=None, xlims=None, ylims=None, binning=1.0, **kwargs):
        ...
        hist_counts, hist_bin, _ = plt.hist(data,
                                            bins=np.arange(data[0], data[-1] + binning, binning),
                                            color='#607c8e', histtype='step')
        plt.xlabel('Times [s]')
        if xlims:
            plt.xlim(*xlims)
        if ylims:
    ...
    def plot_spectra(self, binnum = 64, figsize = None, xlims = None):
        ...
        bins = np.logspace(log10(_instrument_X.en_bins[0]),log10(_instrument_S.en_bins[-1]), binnum)
        mid_energies = [sum(e)/2 for e in energies(self.data)]
        counts, *_ = np.histogram(mid_energies, bins = bins)

        plt.bar(x=bins[:-1], height=counts, width=bins[1:] - bins[:-1], facecolor='white', edgecolor='orange')
        plt.loglog()
        plt.ylabel('N$_E$')
        plt.xlabel('E')
        if xlims:
    ...
    def to_dataframe(self, **kwargs):
        ...
        out = pd.DataFrame({'tte': ttes(data),
                            'pha': chns(data),
                            'flag': flags(data)})
        return out
...
class LCSFromModel(_LightCurveSim):
    def simulate(self, id, *, bkg_scenario = None, orbit_fraction = 0, duration_adjust = 0.):
        """
    ...
    def fit_model_background(self):
        def model_residual(pars, t, data=None):
            t0 = t[0]
            v = pars.valuesdict()
            model = v['c0'] + v['c1'] * (t - t0) + v['c2'] * (t - t0) ** 2 #+ v['c3'] * (t - t0) ** 3
            if data is None:
                return model
            return data - model
        ...
        bkg = [x for x in self.model.data if ((x > lo_bot and x < lo_top) or (x > hi_bot and x < hi_top))]
        lo, hi = np.arange(lo_bot, lo_top, step=self.binning), np.arange(hi_bot, hi_top, step=self.binning)
        ...
        out = minimize(model_residual, fit_params, args=((bkg_bins[1:] + bkg_bins[:-1])/2,), kws={'data': bkg_counts})
        out.model = (lambda t: model_residual(out.params, t, data = None))
        self.bkg_fit = out

    def make_burst_pdf(self):
        lo_top, hi_bot = self.model.tTrigger + self.duration_adjust[0], self.model.tTrigger + self.model.t90 + self.duration_adjust[1]

        signal = [x for x in self.model.data if (x > lo_top and x < hi_bot)]
        burst_counts, burst_bins = np.histogram(signal, np.arange(signal[0], signal[-1], self.binning))
        try:
            bkg_est = self.bkg_fit.model((burst_bins[1:] + burst_bins[:-1])/2)
        except IndexError:
            raise IndexError("I'm failing at background subtraction. Binning parameter possibly exceeding model's t90.")
        burst_counts_minus_bkg = (burst_counts - bkg_est).clip(min=0)

        self.burst_pdf = _pdf(list(burst_counts_minus_bkg / np.sum(burst_counts_minus_bkg)),
    ...
    def plot_model_diagnostic(self, binning = None, *args, **kwargs):
        ...
        plt.axvspan(lo_bot - self.model.tTrigger, lo_top - self.model.tTrigger, alpha=0.2, color='red')
        plt.axvspan(hi_bot - self.model.tTrigger, hi_top - self.model.tTrigger, alpha=0.2, color='red')
        plt.axvline(self.duration_adjust[0],  color='red', linewidth = 1.5, label='model_start')
        plt.axvline(self.model.t90 + self.duration_adjust[1],  color='cornflowerblue', linewidth = 1.5, label='model_end')
        ...
        trig_time_shift_corr = - (self.bkg_fit.model(np.array([0.])) - self.bkg_fit.model(np.array([self.model.tTrigger])))*binning/self.binning
        plt.plot(x - self.model.tTrigger, self.bkg_fit.model(x)*binning/self.binning
                 - trig_time_shift_corr, c = 'k')
        plt.legend()
        return fig
...
class LCSCalibration(_LightCurveSim):
    ...
    """
    def simulate(self, burst_duration, *, burst_shape ='rectangular', bkg_scenario = None, orbit_fraction = 0):
        """
    ...
    def make_burst_pdf(self):
        if self.burst_shape == 'triangular':
            distr_data = np.random.triangular(0, self.burst_duration / 2, self.burst_duration, size=10 ** 6)            # TODO: this pdf are kind of bad.
        elif self.burst_shape == 'rectangular':                                                                         #       No random generator should be involved here.
            distr_data = np.random.uniform(0, self.burst_duration, size=10 ** 6)

        burst_counts, burst_bins = np.histogram(distr_data, bins=np.arange(0, self.burst_duration, self.binning))
        self.burst_pdf = _pdf(list(burst_counts / np.sum(burst_counts)),
                              list(burst_bins))
