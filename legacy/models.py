...
from legacy._paths import filepath_background_sim, filepath_instruments
...
class Band:
    ...
    def _band_f(self, en, constant=1):
        ...
        if en < (self.alpha - self.beta) * self.e_0:
            ...
            return (
                (
                    ((self.alpha - self.beta) * self.e_0 / 100)
                    ** (self.alpha - self.beta)
                )
                * exp(self.beta - self.alpha)
                * ((en / 100) ** self.beta)
                / constant
            )
...
class Instrument:
    ...
    def plot_rmf(self):
        ...
        plt.title(
            "Full energy ($20$ - $4116$ keV) and channel scale ($0$-$1023$)", pad=20
        )
        plt.xlabel("Channel")
