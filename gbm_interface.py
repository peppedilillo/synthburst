...
class _LightCurveGBM(_LightCurve):
    ...
    '''
    def __init__(self,grb_id,filter_range = (50,300)):                                                                  # TODO: Probably this should not filter on init
        '''
        :param grb_id:
        :param filter_range:
    ...
    def get_data(grb_id, filter_range):
        ...
        grb_data = [event[0] for event in grb_data if event[1][0] > loen and event[1][1] < hien]
        return grb_data
    ...
    def get_metadata(grb_id):
        ...
        answer_dict = {}
        metadata = ['id', 'date', 't90', 't90_err', 'tStart', 'tStop', 'tTrigger', 'trig_det', 'fluence', 'fluence_err',
                    'fluenceb', 'fluenceb_err', 'pflx_int', 'pflx', 'pflx_err', 'pflxb', 'lobckint', 'hibckint']
    ...
    def calculate_bkg_interval(self):
        ...
        def retrieve_bck_interval_from_model_metadata():
            lo_bot, lo_top = tuple(map((lambda t: max(min(self.data), t + self.tTrigger)),
                                       [float(val) for val in self.lobckint[1:-1].split(', ')]))
            hi_bot, hi_top = tuple(map((lambda t: min(max(self.data), t + self.tTrigger)),
                                       [float(val) for val in self.hibckint[1:-1].split(', ')]))
            if lo_bot == lo_top or hi_bot == hi_top:
                warnings.warn('\nGRB{} has not enough data for safe background estimation.'.format(self.model.id), stacklevel=2)
        ...
        def retrieve_bck_interval_from_model_t90():
            ...
            margin_lo, margin_hi = calculate_margins()                                                                  # we take a margin from trigger for bkg interval estimate
...
def SQL_insert_data(conn, table, task):
    ...
    """
    cur = conn.cursor()
    colnum = cur.execute('''SELECT count(*) FROM pragma_table_info('GBM_GRB')''').fetchall()[0][0] #get column number
...
def SQL_grb_retrieve_trigdet(conn,grb_id):
    ...
    """
    cur = conn.cursor()
    triglist_unfetch =json.loads(cur.execute("SELECT trig_det FROM GBM_GRB WHERE id =?",(grb_id,)).fetchall()[0][0])
    triglist_fetch = [int(triglist_unfetch[i][4:6]) for i in range(len(triglist_unfetch))]
...
def fetch_grbs(num,seed = None, query = "SELECT id, t90, fluence FROM GBM_GRB WHERE t90 > 1 AND t90 < 4. AND pflxb > 2"):
    import random
